package com.yhl.oauth2.provider.token;
...
/**
 * 当OWNER_APP登录时，始终会刷新token,
 * 对token的管理，读写刷新等
 * token 自定义管理
 * */
public class MyDefaultTokenService  implements AuthorizationServerTokenServices, ResourceServerTokenServices,
     ...
     * If no random generator is set, one will be  created.
     */
    @Override
    public void afterPropertiesSet() throws Exception {
        Assert.notNull(tokenStore,"token 不能为空");
    }
    //创建token
    @Override
    ...
    public OAuth2AccessToken createAccessToken(OAuth2Authentication authentication) throws AuthenticationException {
        ...
        if (existingAccessToken!=null){
            ...
            if (existingAccessToken.isExpired()
                 ||YHL_APP.equals(authentication.getOAuth2Request().getClientId())){

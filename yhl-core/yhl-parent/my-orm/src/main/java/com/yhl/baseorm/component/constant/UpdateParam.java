...
@Component
public class UpdateParam extends LinkedHashMap  {

    /**
     * @param  entity 要跟新的实例
     * @param  updateParams  等于 LinkedHashMap，接收的参数
     * @param entityManager  DAO层传入，主要作用时吧主要时吧引用类的ID转换成为对象
     * @return  T被设置后的实体
     *  //TODO 注意：不管主键的名字交什么,UpdateParams中代表主键的就是id
     * */
    public static <T> T copyPropertis(T entity, UpdateParam updateParams, EntityManager entityManager){
        //获取要跟新的所有字段
        Map<String, Field> map = MyClassUtil.getAllFields(entity.getClass());
        Iterator iterator = updateParams.keySet().iterator();
        try {
        while (iterator.hasNext()){
            ...
            Object object = updateParams.get(key);
     ...
     *  //TODO 注意：不管主键的名字交什么,UpdateParams中代表主键的就是id
     * */
    public static <T> T copyPropertis(T entity, UpdateParam updateParams, EntityManager entityManager, Map<String, Field> map){
        //获取要跟新的所有字段
        Iterator iterator = updateParams.keySet().iterator();
        try {
            while (iterator.hasNext()){
